...
`*_Answer using joins:_*`

_in this approach, I combined joins and subqueries. To be able to filter the films by the most profitable customer at the `WHERE` clause, I recycled the subqueries in xref:Step-2:[Step 2] when building the query using subqueries only._

 _Also this time Im using the `USING` clause, which I find very convenient to setup joins when the joining keys have the same name_
